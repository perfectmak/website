      ...
      klaw(`./src/${type}`)
        ...
        .on('data', (item) => {
          if (path.extname(item.path) === '.md') {
            ...
            if (!categories.includes(dataObj.data.category)) categories.push(dataObj.data.category)
          }
        })
        ...
        .on('end', () => {
          posts = posts.sort(
            (p1, p2) =>
              new Date(p2.data.published_at).getTime() - new Date(p1.data.published_at).getTime()
          )
